...
async function updateDeudor(id, deudor) {
    try {
        ...
        const { nombre, rut, telefono, email } = deudor;
        // const deudorRutFound = await Deudor.findOne({ rut: deudor.rut });

        // const deudorEmailFound = await Deudor.findOne({ email: deudor.email });

        // if (deudorRutFound) return [null, "rut ya existe"];
        // if (deudorEmailFound) return [null, "email ya existe"];
